...
class TrainModel:
    ...
    def filter_nan_values(self, data_dict):
        ...
        filtered_dict = {
            "varNames": [
                name for i, name in enumerate(data_dict["varNames"]) if valid_mask[i]
            ],
            "geomMScoreImportance": [
                score
                for i, score in enumerate(data_dict["geomMScoreImportance"])
                if valid_mask[i]
            ],
            "fBetaScoreImportance": [
                score
                for i, score in enumerate(data_dict["fBetaScoreImportance"])
                if valid_mask[i]
            ],
        }
