...
class TrainModel:
    ...
    def process_client(self, client, df):
        ...
        categorical_vars = list(set(KEEP_VARS) - (set(MULTILABEL_VARS) | set(NUMERICAL_VARS) | set(["metaGeomMScore1", "metaFBetaScore1"])))
    ...
    def train_model(self, df_, n_models=100, n_estimators=10):
        ...
        for metric in ["metaGeomMScore1", "metaFBetaScore1"]:
            ...
            y = df[metric]
            X_processed = X.astype({col: int for col in X.select_dtypes(include=['bool']).columns})
            shap_values_list = []
            ...
            for i in range(n_models):
                ...
                shap_values_list.append(shap_values.values)  # Store full matrix, not just mean
            ...
            shap_values_avg = np.tanh(shap_values_avg * (1/(shap_values_avg.std()*2)))
        ...
        output={
            "varNames": list(importance_scores["metaGeomMScore1_blended_importance"].index),
            "geomMScoreImportance": importance_scores["metaGeomMScore1_blended_importance"].values.tolist(),
            "fBetaScoreImportance": importance_scores["metaFBetaScore1_blended_importance"].values.tolist(),
        }
    ...
    def explode_multi_label(self, df, columns):
        ...
        for column in columns:
            ...
            one_hot = pd.get_dummies(exploded_df[column], prefix=column, prefix_sep='::')
            ...
            duplicate_cols = [col for col in aggregated.columns if col in df_copy.columns]
            if duplicate_cols:
