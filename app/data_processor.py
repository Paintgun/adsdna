...
class DataProcessor:
        ...
        for var in ads.columns:
            if var not in ["metaGeomMScore1", "metaFBetaScore1"]:
                ...
                if var in MULTILABEL_VARS:
                    geomMLongFormat = self.create_long_format_dataframe(client, ads, var, "metaGeomMScore1")
                    fbetaLongFormat = self.create_long_format_dataframe(client, ads, var, "metaFBetaScore1")
                    client_obj["plotData"].append({
                        "varName": var,
    ...
    def create_long_format_dataframe(self, client, df, categorical_column, target_variable):
        """
        ...
        df = df.copy()
        df[categorical_column] = df[categorical_column].apply(lambda x: x if isinstance(x, list) else [])
        ...
        aggregated[target_variable] = df[target_variable].iloc[aggregated.index].reset_index(drop=True)

        # Convert to long format
        long_df = aggregated.melt(id_vars=[target_variable], var_name='Factor', value_name='Indicator')
